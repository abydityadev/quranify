    ...
    return (
        <section>
            ...
            <div className={twclsx("grid grid-cols-2 lg:grid-cols-3 gap-2 my-3")}>
                {sura?.data.map((q) => {
                    return (
                        <Link href={`/surah/${q.nomor}`} className={twclsx('max-w-sm p-6 border rounded-lg shadow-md transition border-gray-700 hover:-translate-y-1')}>
                            <div>
                ...
                <Link href={'/surah'} className={twclsx('text-gray-200')}>view more →</Link>
            </div>
        </section>
    )
}
